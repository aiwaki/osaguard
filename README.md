...
Первая сборка — `v0.1.0-preview.1` для Apple Silicon и macOS 13+. Это
...
версиями. Встроенный updater для первой preview намеренно выключен: следующую
...
2. Скачайте `OsaGuard_0.1.0_preview.1_aarch64.dmg` и перенесите приложение в
   «Программы».
...
установки. Но первая публичная preview **не подключена к updater-каналу**:
